...
public class TestManager {
	...
	/**
	 * @Before public void setUp() throws IOException { cfg=new Configuration();
	 *         cfg.configure("hibernate.cfg.xml");
	 *         factory=cfg.buildSessionFactory(); session=factory.openSession();
	 * 
	 *         Transaction t=session.beginTransaction(); Query<Inquiry>
	 *         query=session.createQuery("delete from Inquiry");
	 *         query.executeUpdate(); query=session.createQuery("delete from
	 *         Sanction"); query.executeUpdate(); t.commit(); }
	 **/
